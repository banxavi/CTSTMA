...
def forgot_pass():
    ...
    if request.method == 'POST':
        ...
        text = """\Hi """
        html = render_template('form_forgot_pass.html', link=link, email=email)
        part1 = MIMEText(text, 'plain')
        part2 = MIMEText(html, 'html')
        msg.attach(part1)
        msg.attach(part2)
        ...
        account = cursor.fetchone()
        if not(account):
            loi = "Tài khoản này không tồn tại"
        else:
            context = ssl.create_default_context()
            with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as server:
                server.login(sender_email, "hoangviet01")
                ...
                return render_template('noti_res.html')
        # return '<h1>The email you entered is {}. The token is {}</h1>'.format(email, token)
    return render_template('forgot_password.html', loi = loi)
...
def logi():
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            'SELECT * FROM employee WHERE email = %s AND password = %s', (tma, password,))
        account = cursor.fetchone()
        # idempl = account[0]
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if account:
            ...
            cursor.execute('insert INTO  Account(NAME, IDNAME, PASSWORD,ADDRESS,CITY,COUNTRY,ROLE) VALUES (%s, %s, %s,%s,%s,%s,%s)',
                           (name, idname, password, address, city, country, role,))
            mysql.connection.commit()
...
def add():
    ...
    if request.method == 'POST':
        ...
        if acc1[7] != "ADMIN":
            ...
            cursor.execute('insert INTO  Account(NAME, IDNAME, PASSWORD,ADDRESS,CITY,COUNTRY,ROLE) VALUES (%s, %s, %s,%s,%s,%s,%s)',
                           (name, idname, password, address, city, country, select,))
            mysql.connection.commit()
...
def done():
    ...
    cursor.execute(
        "UPDATE missionprocess SET status = 'Hoàn thành' WHERE id_process = %s", (idprocess,))
    mysql.connection.commit()
    succ = "UPDATED SUCCESSFUL"
